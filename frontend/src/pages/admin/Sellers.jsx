...
export default function AdminSellers() {
  ...
  const fetchSellers = useCallback(() => {
    setLoading(true)
    adminService.getSellers({ page, size: 10, sort: 'id', email: email || undefined })
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" exit="exit" className="space-y-5">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Sellers</h1>
      ...
      {loading ? (
        ...
        <EmptyState icon={Store} title="No sellers found" description="No sellers match your search" />
      ) : (
        <>
                      ...
                      <td>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">{s.firstName} {s.lastName}</p>
                          </div>
                        </div>
                      </td>
                      <td className="text-gray-500">{s.email}</td>
                      <td className="text-gray-500 font-mono text-xs">{s.gst || '—'}</td>
                      <td className="text-gray-500">{s.companyContact || '—'}</td>
                      <td>
                        <Badge variant={s.active ? 'success' : 'danger'}>{s.active ? 'Active' : 'Inactive'}</Badge>
                      </td>
                      <td>
                        {s.active ? (
                          <button onClick={() => setConfirmAction({ id: s.id, action: 'deactivate', name: s.companyName })}
                            className="btn-sm btn text-red-600 hover:bg-red-50 dark:hover:bg-red-950 gap-1">
                            <UserX size={14} /> Deactivate
                          </button>
                        ) : (
                          <button onClick={() => setConfirmAction({ id: s.id, action: 'activate', name: s.companyName })}
          ...
          <Pagination page={page} onPageChange={setPage} hasNext={sellers.length === 10} />
        </>
      )}
