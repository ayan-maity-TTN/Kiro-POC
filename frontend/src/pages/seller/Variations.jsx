...
export default function SellerVariations() {
  ...
  const [form, setForm] = useState({ price: '', quantityAvailable: '', metadata: '' })
  ...
  const openAdd = () => { setForm({ price: '', quantityAvailable: '', metadata: '' }); setEditId(null); setImagePreview(null); setShowForm(true) }
  const openEdit = (v) => {
    setForm({ price: v.price || '', quantityAvailable: v.quantityAvailable || '', metadata: JSON.stringify(v.metadata || {}) })
    setEditId(v.id)
    setImagePreview(v.imageUrl || null)
    setShowForm(true)
  }
  const closeForm = () => { setShowForm(false); setEditId(null); setImagePreview(null) }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.metadata) {
        try { JSON.parse(form.metadata); formData.append('metadata', form.metadata) } catch { toast.error('Invalid metadata JSON'); setSaving(false); return }
      }
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" exit="exit" className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/seller/products')} className="btn-icon btn-ghost"><ArrowLeft size={18} /></button>
        <div>
          <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Product Variations</h1>
          {product && <p className="text-sm text-gray-500 dark:text-gray-400">{product.name}</p>}
        </div>
        <button onClick={openAdd} className="btn-primary btn-sm ml-auto"><Plus size={16} /> Add Variation</button>
      </div>
        ...
        {showForm && (
          <motion.div variants={fadeInUp} initial="hidden" animate="visible" exit="exit" className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-semibold text-gray-900 dark:text-white">{editId ? 'Edit Variation' : 'Add Variation'}</h2>
              <button onClick={closeForm} className="btn-icon btn-ghost"><X size={18} /></button>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <FormField label="Price *">
                <input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })}
                  placeholder="0.00" className="input" min="0" step="0.01" />
              </FormField>
            ...
            <FormField label="Metadata (JSON)" className="mt-4">
              <textarea value={form.metadata} onChange={(e) => setForm({ ...form, metadata: e.target.value })}
                rows={3} placeholder='{"color": "red", "size": "M"}' className="input resize-none font-mono text-xs" />
            </FormField>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <button type="button" onClick={() => fileRef.current?.click()} className="btn-secondary btn-sm">
                    <Image size={14} /> {imagePreview ? 'Change Image' : 'Upload Image'}
                  </button>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={handleSave} disabled={saving} className="btn-primary">
                {saving ? 'Saving...' : <span className="flex items-center gap-2"><Save size={16} /> {editId ? 'Update' : 'Add'} Variation</span>}
              </button>
      ...
      {loading ? (
        ...
        <EmptyState icon={Plus} title="No variations yet" description="Add product variations with different prices and attributes"
          action={{ label: 'Add Variation', onClick: openAdd }} />
      ) : (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible"
          className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {variations.map((v) => (
            <motion.div key={v.id} variants={staggerItem} className="card overflow-hidden">
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-gray-900 dark:text-white">{formatCurrency(v.price || 0)}</span>
                  <span className={`badge ${v.isActive ? 'badge-success' : 'badge-danger'}`}>{v.isActive ? 'Active' : 'Inactive'}</span>
                </div>
                <p className="text-sm text-gray-500">Qty: {v.quantityAvailable || 0}</p>
                {v.metadata && Object.keys(v.metadata).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {Object.entries(v.metadata).map(([k, val]) => (
                      <span key={k} className="badge badge-primary text-xs">{k}: {val}</span>
                    ))}
                  </div>
                )}
                <button onClick={() => openEdit(v)} className="btn-secondary btn-sm w-full mt-3">
                  <Edit2 size={14} /> Edit
                </button>
              </div>
            </motion.div>
