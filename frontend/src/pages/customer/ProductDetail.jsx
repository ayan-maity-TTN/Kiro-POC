...
import { ShoppingCart, Heart, Star, Package, RefreshCw, Shield, Truck, Send } from 'lucide-react'
...
export default function ProductDetail() {
    ...
    Promise.all([
      ...
      .then(([productRes, similarRes, reviewsRes, wishlistRes]) => {
        ...
        if (p?.variations?.length > 0) {
          setIsWishlisted(wishlist.some((w) => w.variationId === p.variations[0].id))
        }
      })
  ...
  const handleAddToCart = async () => {
    ...
    setCartLoading(true)
    try {
      await customerService.addToCart({ productVariationId: selectedVariation.id, quantity: 1 })
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await customerService.addReview({ productId: Number(id), review: reviewText, rating: reviewRating })
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" exit="exit" className="space-y-8">
      <Breadcrumb items={[
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {/* Image */}
        <motion.div variants={fadeInUp} className="rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800 aspect-square">
          {selectedVariation?.imageUrl ? (
            <img src={selectedVariation.imageUrl} alt={product.name} className="w-full h-full object-contain" />
          ) : (
        ...
        <motion.div variants={fadeInUp} className="space-y-5">
          <div>
            ...
            <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">{product.name}</h1>
            {avgRating && (
              <div className="flex items-center gap-2 mt-2">
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map((s) => (
                    <Star key={s} size={14} className={s <= Math.round(Number(avgRating)) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {variations.map((v) => (
                  <button key={v.id}
                    ...
                    {v.metadata ? Object.values(v.metadata).join(' / ') : `Variant ${v.id}`}
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* Stock */}
          {selectedVariation && (
            <p className={`text-sm font-medium ${selectedVariation.quantityAvailable > 0 ? 'text-green-600' : 'text-red-500'}`}>
              {selectedVariation.quantityAvailable > 0 ? `In Stock (${selectedVariation.quantityAvailable} left)` : 'Out of Stock'}
            </p>
          )}
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={cartLoading || !selectedVariation || selectedVariation?.quantityAvailable === 0}
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            {[
              ...
              { icon: RefreshCw, text: product.isReturnable ? 'Returnable' : 'Non-Returnable' },
              { icon: Package, text: product.isCancellable ? 'Cancellable' : 'Non-Cancellable' },
            ].map((f, i) => (
      ...
      <div className="space-y-6">
        ...
        <div className="card p-5">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 text-sm">Write a Review</h3>
          <form onSubmit={handleSubmitReview} className="space-y-3">
              ...
              <div className="flex gap-1">
                {[1,2,3,4,5].map((s) => (
                  <button key={s} type="button" onClick={() => setReviewRating(s)}>
                    <Star size={20} className={s <= reviewRating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 hover:text-yellow-300'} />
                  </button>
                ))}
              </div>
            </div>
            ...
            />
            <button type="submit" disabled={reviewLoading} className="btn-primary btn-sm">
              {reviewLoading
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-0.5 mt-0.5">
                        {[1,2,3,4,5].map((s) => (
                          <Star key={s} size={11} className={s <= Number(r.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />
                        ))}
                      </div>
                    </div>
                  </div>
                </div>
                {r.review && <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 ml-12">{r.review}</p>}
              </motion.div>
            ))}
          </motion.div>
        ) : (
      ...
      {similar.length > 0 && (
        <div>
          <h2 className="text-xl font-display font-bold text-gray-900 dark:text-white mb-4">Similar Products</h2>
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}
            className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {similar.map((p) => (
              <motion.div key={p.id} variants={staggerItem}>
                <Link to={`/customer/product/${p.id}`}
                  className="card block hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                  <div className="aspect-square bg-gray-100 dark:bg-gray-800">
                    {p.imageUrls?.[0] ? (
                      <img src={p.imageUrls[0]} alt={p.name} className="w-full h-full object-cover" loading="lazy" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-gray-300"><Package size={24} /></div>
                    )}
                  </div>
                  <div className="p-3">
                    <p className="text-xs font-medium text-gray-900 dark:text-white line-clamp-2">{p.name}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
