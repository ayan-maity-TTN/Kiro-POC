...
import { Search, SlidersHorizontal, ChevronDown, Star, Heart, ShoppingCart, X } from 'lucide-react'
...
function ProductCard({ product }) {
  ...
  const handleAddToCart = (e) => {
    e.preventDefault()
    dispatch(addToCart({ variationId: product.id, name: product.name, price: product.price, quantity: 1 }))
    toast.success('Added to cart!')
  }

  const handleWishlist = (e) => {
    e.preventDefault()
    dispatch(toggleWishlist({ variationId: product.id, name: product.name, price: product.price }))
    toast.success(isWishlisted ? 'Removed from wishlist' : 'Added to wishlist!')
  }

  return (
    <motion.div variants={staggerItem} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
      <Link to={`/customer/product/${product.id}`} className="card block group overflow-hidden">
        ...
        <div className="p-4">
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-1 truncate">{product.brand}</p>
          <h3 className="font-medium text-gray-900 dark:text-white text-sm leading-snug line-clamp-2 mb-2">{product.name}</h3>
          <div className="flex items-center gap-1 mb-3">
            {[1,2,3,4,5].map((s) => (
              <Star key={s} size={12} className={s <= 4 ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />
            ))}
            <span className="text-xs text-gray-400 ml-1">(4.0)</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-bold text-gray-900 dark:text-white">{formatCurrency(product.price || 0)}</span>
...
export default function CustomerProducts() {
  ...
  const fetchProducts = useCallback(() => {
    ...
    customerService.getProducts({ page, size: 12, sort: sortField, order: sortOrder, query: search || undefined, categoryId })
    ...
    <motion.div variants={pageTransition} initial="hidden" animate="visible" exit="exit" className="space-y-5">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button onClick={() => setShowFilters(!showFilters)} className="btn-secondary btn-icon">
      ...
      {loading ? (
        ...
        <EmptyState icon={ShoppingCart} title="No products found" description="Try adjusting your search or filters" />
      ) : (
      ...
      {products.length > 0 && (
        <Pagination page={page} onPageChange={setPage} hasNext={products.length === 12} />
      )}
    </motion.div>
  )
}
