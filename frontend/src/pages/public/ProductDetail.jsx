...
export default function PublicProductDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      publicService.getSimilarProducts({ productId: id, limit: 6 }),
      publicService.getProductReviews(id).catch(() => ({ data: [] })),
    ])
      .then(([productRes, similarRes, reviewsRes]) => {
  ...
  const requireAuth = () => {
    if (!isAuthenticated) {
      ...
      navigate("/login", {
        state: { from: { pathname: `/product?productId=${id}` } },
      });
      return true;
    }
    return false;
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      setReviewText("");
      setReviewRating(5);
      const res = await publicService.getProductReviews(id);
      setReviews(res.data || []);
    } catch (err) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      exit="exit"
      className="page-container py-8 space-y-8"
    >
      <Breadcrumb
        items={[
          { label: "Home", to: "/" },
