...
import { ShoppingBag, Shield, Truck, RefreshCw, Star, ArrowRight, Zap, Award, Users } from 'lucide-react'
import { fadeInUp, staggerContainer, staggerItem, pageTransition, scaleIn } from '../../animations/variants'
...
const features = [
  { icon: Truck, title: 'Free Delivery', desc: 'On orders above ₹499', color: 'text-blue-500' },
  { icon: Shield, title: 'Secure Payment', desc: '100% secure transactions', color: 'text-green-500' },
  { icon: RefreshCw, title: 'Easy Returns', desc: '7-day hassle-free returns', color: 'text-orange-500' },
  { icon: Award, title: 'Quality Assured', desc: 'Verified sellers only', color: 'text-purple-500' },
]

const testimonials = [
  { name: 'Priya Sharma', role: 'Regular Customer', text: 'Amazing shopping experience! Fast delivery and great quality products.', rating: 5, avatar: 'PS' },
  { name: 'Rahul Verma', role: 'Verified Buyer', text: 'Best prices in the market. The seller support is excellent too.', rating: 5, avatar: 'RV' },
  { name: 'Anita Singh', role: 'Premium Member', text: 'Love the variety of products. Easy returns make shopping stress-free.', rating: 4, avatar: 'AS' },
]
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" exit="exit">
      ...
      <section className="relative">
        ...
        >
          {heroSlides.map((slide, i) => (
            <SwiperSlide key={i}>
              <div className={`relative h-full bg-gradient-to-r ${slide.bg} flex items-center overflow-hidden`}>
                <div className="absolute inset-0 opacity-20">
                  <img src={slide.img} alt="" className="w-full h-full object-cover" loading="lazy" />
                </div>
                <div className="relative page-container w-full">
                  <motion.div
                  ...
                  >
                    <h1 className="text-4xl md:text-5xl font-display font-bold leading-tight mb-4">{slide.title}</h1>
                    <p className="text-lg text-white/80 mb-8">{slide.subtitle}</p>
                    <div className="flex gap-4">
                      <Link to="/register/customer" className="btn bg-white text-primary-700 hover:bg-gray-100 btn-lg font-semibold shadow-lg">
                        {slide.cta} <ArrowRight size={18} />
                      </Link>
                      <Link to="/about" className="btn border-2 border-white text-white hover:bg-white/10 btn-lg">
                        Learn More
                      </Link>
                    </div>
                  </motion.div>
        ...
        <div className="page-container py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {features.map((f, i) => (
              ...
              >
                <div className={`p-2 rounded-xl bg-gray-50 dark:bg-gray-800 ${f.color}`}>
                  <f.icon size={22} />
                </div>
                <div>
      ...
      <section className="section bg-gray-50 dark:bg-gray-950">
        <div className="page-container">
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-10">
            <h2 className="text-3xl font-display font-bold text-gray-900 dark:text-white mb-2">Shop by Category</h2>
      ...
      <section className="section bg-white dark:bg-gray-900">
        <div className="page-container">
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-10">
            <h2 className="text-3xl font-display font-bold text-gray-900 dark:text-white mb-2">What Our Customers Say</h2>
            <p className="text-gray-500 dark:text-gray-400">Trusted by millions across India</p>
          </motion.div>
          <Swiper
              ...
              <SwiperSlide key={i}>
                <div className="card p-6 h-full">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full hero-gradient flex items-center justify-center text-white font-bold text-sm">{t.avatar}</div>
                    <div>
      ...
      <section className="section bg-gray-50 dark:bg-gray-950">
        <div className="page-container">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4 relative">Ready to Start Shopping?</h2>
            <p className="text-white/80 text-lg mb-8 relative">Join millions of happy customers. Register today and get exclusive deals.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center relative">
              <Link to="/register/customer" className="btn bg-white text-primary-700 hover:bg-gray-100 btn-lg font-semibold shadow-lg">
                Register as Customer <ArrowRight size={18} />
              </Link>
              <Link to="/register/seller" className="btn border-2 border-white text-white hover:bg-white/10 btn-lg">
                Become a Seller
              </Link>
            </div>
          </motion.div>
